...
function drawChart() {
  ...
  d3.csv("BEZ_Zeitpunkt_Gesuchseinreichung_2025.csv").then(raw => {
    ...
    data.forEach((d, i) => {
      ...
      // ⭐ Quand les deux points sont très proches (valeurs voisines, ex. 718
      // et 808), les deux étiquettes au-dessus peuvent se toucher : on
      // vérifie leur écart réel et on les écarte symétriquement si besoin.
      const leftBBox = leftLabel.node().getBBox();
      const rightBBox = rightLabel.node().getBBox();
      const minGap = 8;
      const overlap = (leftBBox.x + leftBBox.width + minGap) - rightBBox.x;
      if (overlap > 0) {
        leftLabel.attr("x", xMin - overlap / 2);
        rightLabel.attr("x", xMax + overlap / 2);
      }

      // ⭐ Ne jamais déborder dans la zone des labels de ligne à gauche.
      const leftBBoxAfter = leftLabel.node().getBBox();
      if (leftBBoxAfter.x < 10) {
        leftLabel.attr("x", +leftLabel.attr("x") + (10 - leftBBoxAfter.x));
      }
