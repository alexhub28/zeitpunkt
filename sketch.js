...
function drawChart() {
  ...
  d3.csv("BEZ_Zeitpunkt_Gesuchseinreichung_2025.csv").then(raw => {
    ...
    data.forEach((d, i) => {
      ...
      // mais JAMAIS au point de recouvrir le point lui-même (rayon 7px) :
      // le recalage est plafonné pour toujours rester à sa gauche.
      const leftBBox = leftLabel.node().getBBox();
      let leftX = xMin - 16;
      if (leftBBox.x < 10) {
        leftX = (xMin - 16) + (10 - leftBBox.x);
      }
      const maxLeftX = xMin - 11; // bord du point (rayon 7) + marge 4
      if (leftX > maxLeftX) leftX = maxLeftX;
      leftLabel.attr("x", leftX);
